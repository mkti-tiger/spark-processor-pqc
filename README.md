...
これにより、L1改造なしでL2サービス層を構築することが可能です。この標準機能を最大限に活用し、量子耐性（PQC）署名を統合しています。
Transfer Transaction内のPlain Messages
SymbolのTransfer Transactionでは、最大1,023バイトの非暗号化文字列ペイロードであるPlain Messageを含めることができます。
これにより、JSONオブジェクトなどの構造化データを文字列形式にシリアライズして直接オンチェーンに記録可能です。
...
ネットワークへのアナウンス後、このデータはトランザクションハッシュおよび関連ブロックを通じて検証可能かつ不変的に記録されます。
この機能はSymbolのネイティブ仕様であり、L1の変更は一切必要ありません。
バッチ処理のためのAggregate Transactions


Aggregate Transactionは、複数の内包（Embedded）Transaction（例：Plain Message付きTransfer Transaction）を単一の原子単位に集約します。
Symbolパブリックネットワークでは最大100個の内包トランザクションをサポートし、「すべて成功またはすべて失敗」の原子性を保証します。

Aggregateはイニシエーターによる1回の署名（マルチシグ時は追加cosignature）で処理され、署名オーバーヘッドを大幅に削減します。
従来の署名をDilithium（またはML-DSA-65）などのPQCアルゴリズムに置き換えることで、この単一署名を内包トランザクションハッシュのMerkle Rootを含むAggregateペイロード全体に適用します。
これにより、オンチェーンストレージ容量・ネットワーク手数料を最小化しつつ、原子性と量子耐性を両立します。


PQC署名の統合
Docker Plugin内で以下の処理フローを実装することで、L1を改造せずに量子耐性サービスを実現しています


E2E暗号化リクエスト経由でトランザクションハッシュを蓄積
Merkle Treeを構築し、圧縮のためのRootハッシュを導出
Plain MessageにJSONデータを埋め込み、Aggregate全体を構築
Aggregate全体に対して1回のPQC署名（Dilithium / ML-DSA-65など）を適用
Symbol Node（localhost REST API）へアナウンスし、結果をE2E暗号で返信


実動作例：Symbol初の量子耐性PQC署名アンカー記録
KICN-FT（@kicnft.symbol）氏により、2026年2月18日におそらくSymbolブロックチェーン史上初の量子耐性PQC（ML-DSA-65）署名アンカーが記録されました。
トランザクション詳細


タイプ: Transfer with Plain Message
Payload JSON: 上記例と同一
...
PQC署名→マークルツリーとなっているが作図したAIアシスタントの間違いです。

これでは、容量削減できません。

正しくは、

E2E経由で届いたハッシュを貯めて行ってから、

マークルツリーで圧縮してマークルルートを作ります、

ルートが複数集まったら、

SYMBOLのTXを集約して送信する機能アグリゲートでまとめて送る時にPQC署名（Plain MessagesにJSONデータを埋め込み）、

PQC署名が1回で済むから容量・料金が大幅に減少させることが可能です。

結果をE2E経由で返信します。

ここが「SYMBOLの強み」となります。

現在β版READMEから、仕様書β版を作る時に修正予定です。
