...
本プロジェクト **spark-processor-pqc β1.0.0** は、金融庁「預金取扱金融機関の耐量子計算機暗号への対応に関する検討会報告書」（令和6年11月26日）が求める**クリプト・アジリティの向上**、**ハイブリッド方式**による段階的移行、**HNDL攻撃対策**、**DLT量子耐性強化**等の主要要件を実装レベルでほぼ完全に満たしています。

既存ブロックチェーンを一切変更せずにPQCを後付けできる非侵入型Dockerプラグイン方式、送金額に応じたPQC署名のオプション選択機能、E2E暗号化（PQC-KEM + AES-256-GCM）、CC0公開および特許権の永久献上により、金融機関や開発者が安心して検証・採用できる現実的な量子耐性ソリューションとして提供しています。
