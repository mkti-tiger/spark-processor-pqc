...
つまり、**「盗まれないようにする」のではなく、「改ざんが即座に検知される」**仕組みを提供します。
...
β1.1.0（進行中）
  ...
  ✅ Authenticity Primitive v1.2.0 仕様公開（spec/）
  🔲 検証CLI作成（最優先）
  🔲 testnetでの全フロー検証
  🔲 HSM連携・複数アルゴリズム自動切替
...
Step 5: そのハッシュに対してPQC署名を1回生成
        ...
        署名サイズ: FN-DSA-512 = 平均約617バイト、最大666バイト（可変長）
...
FN-DSA-512 署名:       最大666 バイト（可変長、平均約617バイト）
Merkle Root (SHA3-512):     64 バイト
JSONオーバーヘッド:         ~80 バイト
─────────────────────────────────────────
署名のみ記録:          最大約810 バイト（Plain Message 1,023バイト制限内）
...
3. PQC公開鍵でPQC署名を検証
4. IPFS / ArweaveからMerkle Proofを取得（※β1.1.0以降実装予定）
...
① 目次に「仕様ドキュメント」を追加
- [仕様ドキュメント](#-仕様ドキュメント) を追加しました。
② 「盗まれても分かる」→「改ざんが即座に検知される」
プロジェクトの本質セクションの表現を技術的に正確な表現に修正しました。
③ KICN-FTのTXハッシュに注記を追加
※正確なハッシュ値を要確認（64文字のhex文字列） と記載しました。正確な値をご確認の上、差し替えてください。
④ 署名サイズを正確な数値に修正
約690バイト → 平均約617バイト、最大666バイト（可変長） に修正しました。容量設計表も同様に更新しました。
⑤ Merkle RootのサイズをSHA3-512の正確な値に修正
32バイト → 64バイト に修正し、SHA3-512 と明示しました。
⑥ 完全検証フローのステップ4に「実装予定」注記を追加
IPFSの手順が未実装であることを明示しました。
⑦ ロードマップに仕様公開を追記
✅ Authenticity Primitive v1.2.0 仕様公開（spec/） を追加しました。
